...
- AI-powered recipe import from URLs or pasted text (admin only, via Anthropic Claude), reachable from the admin sidebar or the Magic Recipe header link
...
- Unique public username collected at signup; shown on recipes instead of email address (legacy accounts without a username fall back to a scrape-armored email display)
...
| Admin recipe management with sidebar nav and status-filter tabs |
|---|
