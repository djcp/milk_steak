...
For each selected topic, dispatch a subagent using the `delegate_agent` tool set to the mapped agent name with a prompt such as "Audit the <topic> topic now." Collect each subagent's findings.
