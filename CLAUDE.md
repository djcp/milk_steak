...
- **Solid Cache** for `Rails.cache` in production (separate cache database, `db/cache_schema.rb`, tuned in `config/cache.yml`). Production previously had `perform_caching = true` with `cache_store` unset, which silently meant a per-process, restart-volatile `:memory_store`
...
- `app/models/filter_set.rb` — PORO (ActiveModel::Model) for compound recipe filtering (tags, name, ingredients, author); author filter uses `users.username ilike ?` (case-insensitive partial match). `apply_to` only appends `.distinct` when a join-based filter (tag or ingredient) is active — name/author filters and the unfiltered public index don't need the extra sort/hash pass
...
- `app/controllers/users/sessions_controller.rb` / `registrations_controller.rb` — Devise overrides that rate-limit `#create` (10 per 3 min for sessions, 10 per 10 min for registrations) via `rate_limit ... with: :rate_limited`, which **redirects back to the form with the `devise.failure.too_many_requests` alert** (not a 429 — earlier docs said otherwise). Rails counts through `Rails.cache`, so this is inert under a `:null_store`; the test environment therefore uses `:memory_store`, cleared between examples
- `app/controllers/admin/base_controller.rb` — **Does not enforce admin** (see the note at the top of the file): `admin/recipes` and `admin/ai_classifier_runs` are a shared shell that members reach, seeing only their own records via `policy_scope`. A new admin-only controller must opt in with `before_action :require_admin!`. Provides `layout 'admin'`, `before_action :require_logged_in_approved!`, `after_action :verify_authorized`, and `require_admin!` (`authorize :site, :admin_area?`). Admin auth is a **policy** check, not a visibility shortcut
...
- `app/controllers/admin/ai_classifier_runs_controller.rb` — AI run history. `assign_stats` computes total/success/failure counts and mean duration in a **single** pass using `COUNT(*) FILTER (WHERE ...)` rather than four serial aggregates; deliberately uncached, since the page exists to observe recent runs and stale numbers would defeat it. Every query goes through `policy_scope(AiClassifierRun)` (runs of the visible recipe scope, so members only see their own runs read-only); show/rerun `find_run` is scoped (others' runs 404); rerun re-enqueues MagicRecipeJob and is admin-only (`authorize @run, :rerun?`)
...
### Services
- `app/services/safe_url_fetcher.rb` — SSRF-safe HTTP fetch. Resolves DNS, blocks internal/private/link-local ranges (`0.0.0.0/8`, `10/8`, `100.64/10`, `127/8`, `169.254/16` incl. AWS metadata, `172.16/12`, `192.168/16`, `::1`, `fc00::/7`, `fe80::/10`), raises `SafeUrlFetcher::BlockedAddressError`; max 5 redirects, timeouts (5s open / 15s read), `MAX_BYTES = 5.megabytes` enforced **during** the read via the `SizeLimitedBody` String subclass passed to `read_body` (plus a `Content-Length` pre-check), so a hostile server can't buffer the whole payload into memory first — every response is read through the cap, redirects included, because Net::HTTP drains any unread body on block exit. Oversize raises `ResponseTooLargeError`, a subclass of `BlockedAddressError` so existing rescuers still work. **Known gap:** DNS is resolved for validation and again by `Net::HTTP` on connect, so a TOCTOU rebind is still possible; deferred deliberately since pinning to a validated IP requires hand-rolled SNI/`verify_hostname` handling. Test seam: `SafeUrlFetcher.addresses_for` / `blocked_address?`
- `app/services/recipe_ai_extractor.rb` — Sends source content to Anthropic API, returns parsed JSON; wraps text in `<untrusted_recipe_text>` delimiters (prompt-injection defense), stripping any `<untrusted_recipe_text>`/`</untrusted_recipe_text>` occurrence out of the untrusted text first so a scraped page can't close the boundary early and have what follows read as trusted instructions; `validate_result!` requires name+directions and an ingredients array (max 200)
...
### Jobs
- `app/jobs/magic_recipe_job.rb` — Background job for AI recipe processing pipeline. `retry_on StandardError` (3 attempts) with a `discard_on` list declared **after** it — ActiveJob searches rescue handlers in reverse registration order, so the later declaration wins — covering failures that can never succeed on retry (`SafeUrlFetcher::BlockedAddressError`/`TooManyRedirectsError`/`RedirectWithoutLocationError`, `ActiveRecord::RecordNotFound`, `ArgumentError`). The `rescue` inside `#perform` still records `processing_failed` before re-raising either way. `RecipeTextExtractor::FetchError` and `RecipeAiExtractor::InvalidResponse` stay retryable — an upstream 503 and non-deterministic LLM output can both succeed on a second attempt
...
- `admin/users` — index, plus member route: approve (patch); the approve lookup is scoped to `where(approved: false, admin: false)`, matching the index, so anything outside that set 404s
...
- Feature specs use Selenium headless Chrome (`js: true`); default driver is `rack_test`. **Reach for `js: true` only when JavaScript is the thing under test** — server-rendered markup runs identically under `rack_test` at a fraction of the cost. Today that means the recipe form (ingredient-row cloning, autocompletes) in `user_manages_recipes_spec.rb`, the two JS scenarios nested inside `user_filters_recipes_spec.rb`, and `test_harness_spec.rb`; everything else in `spec/features/` is `rack_test`
- `app/assets/javascripts/test_setup.js` (loaded only in test, from `app/views/application/_javascript.html.erb`) sets `$.fx.off` and `$.ajaxSetup({ async: false })`. **It must keep `defer: true`** — jQuery above it is deferred, so an undeferred include runs before jQuery exists, throws `$ is not defined`, and silently applies neither setting. That was the state of things for months, and the largest single contributor to the Selenium flake. Synchronous XHR is deliberate: jQuery UI cannot abandon an in-flight autocomplete request, so an async response can reopen a menu that was already closed. `spec/features/test_harness_spec.rb` asserts both settings actually took
- `dismiss_autocompletes` (`Features::AutocompleteHelpers`) blurs every autocomplete field before a navigation. jQuery UI's blur handler does `clearTimeout(this.searching)` *and* `close()`, so it cancels a search that hasn't fired yet as well as dismissing an open menu — a menu left open across a navigation makes Selenium raise "Node with given id does not belong to the document". `RecipeOnPage#submit` calls it, so every recipe-form spec is covered without doing anything. It replaced a `wait_for_ajax` that polled `jQuery.active` and could not work: jQuery UI waits `options.delay` (300 ms) *before* issuing the request, so a poll straight after a `fill_in` saw nothing in flight and returned immediately
- `spec/support/features/detached_node_retries.rb` translates chromedriver's detached-node `UnknownError` into `StaleElementReferenceError`, which is in `Capybara::Selenium::Driver#invalid_element_errors` and therefore retried inside `synchronize`. A safety net for the residual, not a substitute for awaiting navigations
- `DatabaseCleaner.clean` is registered with **`append_after`**, not `after`. RSpec runs plain `after` hooks in reverse registration order, and Capybara registers its `reset_sessions!` hook earlier (at `require 'rspec/rails'`), so a plain `after` here truncated tables while the browser session was still open. Capybara's reset drains the Puma server's pending requests, but not airtight-ly: a request already on the wire can land just after the drain and deadlock TRUNCATE's `AccessExclusiveLock` against its `RowExclusiveLock`, so `clean` retries up to three times on `ActiveRecord::Deadlocked`. That retry is mitigation for a residual race, not a fix for it
- `user_logs_in` (in `Features::SessionHelpers`) creates and logs in a regular user; `log_in_as(user)` logs in a pre-created user (use this when you need to supply your own user, e.g. an admin). `user_logs_in` delegates to `log_in_as`, which **waits for the post-login page** before returning — clicking submit only starts the POST and redirect, and the caller's next DOM query would otherwise race the navigation
- `RecipeGenerator#create_recipes` builds its two published recipes with FactoryBot. It used to drive the browser through two complete form submissions, which made every consumer a Selenium spec and dragged the whole autocomplete race surface into specs that weren't testing autocompletes. The form has its own coverage in `user_manages_recipes_spec.rb`
...
- The test environment uses `config.cache_store = :memory_store` (not `:null_store`) because Rails' `rate_limit` counts through the cache; `spec_helper` clears the cache before each example so counters can't leak between specs and throttle an unrelated test
- `Devise::Test::IntegrationHelpers` (`sign_in`) is available in request specs, and `ActiveSupport::Testing::TimeHelpers` (`travel`) everywhere
- Request specs live in `spec/requests/`: `accessibility_spec.rb` (Nokogiri-parsed a11y assertions), `recipes_eager_loading_spec.rb` (query-count invariance as an N+1 guard), `rate_limiting_spec.rb`, and `authentication_spec.rb` (the approval gate end to end, which controller specs cannot cover because `sign_in_user` stubs `current_user`)
...
- **Portable knowledge** lives in `.github/skills/<topic>/SKILL.md` — eight audit skills: `security-audit` (folds in Brakeman), `accessibility-audit`, `performance-audit`, `compatibility-audit`, `rails-practices-audit` (folds in RuboCop), `data-integrity-audit`, `dependency-audit` (folds in Bundler Audit), and `test-quality-audit` (coverage gaps, plus determinism/flakiness, suite efficiency, and low-value or duplicated tests — the checks that would have caught the 2026-09 Selenium flake).
...
- Nested attributes: Recipe accepts nested RecipeIngredients and Images. The `recipe_ingredients` `reject_if` keeps a row that supplies only a name — "cilantro" and "salt to taste" are legitimate ingredients with neither quantity nor unit, and were previously discarded on save with no feedback
...
- Image uploads validated for type (JPEG, PNG, WebP, AVIF, HEIC/HEIF) and size (max 10MB). **Variants are transcoded to JPEG** (`format: :jpg`) — HEIC/HEIF render in no browser and AVIF fails on Safari < 16.4, and image_processing preserves the source format unless told otherwise, so uploads in those formats previously produced a broken image for every viewer
...
- A partial unique index (`images.recipe_id WHERE featured = true`) allows at most one featured image per recipe, so `FeaturedImageChooser`'s pick is deterministic. There is deliberately **no** unique index on `recipe_ingredients (recipe_id, section, position)`: `acts_as_list` swaps positions across separate UPDATE statements, so the intermediate state duplicates a position, and Postgres checks a unique index immediately (only a table CONSTRAINT can be `DEFERRABLE`, which the COALESCE expression over the nullable `section` cannot be). Adding it breaks `move_lower`/`move_higher`.
- `users` has functional unique indexes on `lower(username)` and `lower(email)`, matching the model's case-insensitive uniqueness validations; the `''` default on `username` is gone
...
- `ai_classifier_runs` carries composite indexes `(recipe_id, started_at DESC)` and `(success, started_at DESC)`. The former serves both the grouped pagination and the per-recipe re-select; the latter replaced a standalone index on the `success` boolean, which was near-useless at two distinct values.
...
- Use `Recipe.includes(...)` for eager loading associations (not `Preloader`); image blobs and variant records are eager-loaded via `includes(images: { image_attachment: { blob: :variant_records } })` in the recipes controller index/show, and `#index` also preloads `:user`. `Recipe` orders `has_many :images` (`:id`) and `has_many :recipe_ingredients` (`:position`) at the association, so views must **not** call `.order` on them — doing so discards the eager-loaded collection and re-queries
- `spec/requests/recipes_eager_loading_spec.rb` guards this: it asserts query counts are *invariant to collection size* (not an absolute budget, which would drift). Note the `:user` preload on `#index` is defensive — the card doesn't render the author today — and the tag preloads on `#show` are query-neutral for a single recipe; both exist so a future view change fails loudly under `strict_loading_by_default` instead of shipping an N+1 to production, where strict loading is off
...
- `app/controllers/concerns/paginatable.rb` — shared `page_param` / `per_page_param(default:, max:)` clamping for the three paginated indexes, which each hand-rolled it with different maximums. `Admin::RecipesController` previously passed `params[:page]` through unclamped.
...
- Login/registration hardening: Devise `:lockable` locks accounts after 10 failed attempts (email unlock); `users/sessions` and `users/registrations` controllers `rate_limit` `#create` (10/3min and 10/10min), redirecting with the `devise.failure.too_many_requests` alert. Covered by `spec/requests/rate_limiting_spec.rb`, including window expiry and the fact that throttling precedes credential checking.
- `config.paranoid = true` closes user enumeration on the password-reset, confirmation-resend and unlock-resend endpoints. It does **not** affect sign-up, where "Email has already been taken" comes from `:validatable`'s uniqueness validator — an accepted `:registerable` limitation, deliberately kept so users aren't misled into thinking a duplicate registration succeeded.
- `config.password_length = 12..128`. The sign-up hint interpolates `@minimum_password_length` rather than hardcoding it, matching `devise/passwords/edit`.
- `config.hosts` is set from `ENV['HOST']` in production (with `/up` excluded) for DNS-rebinding protection; `config/initializers/host_check.rb` guarantees `HOST` is present by raising at boot.
- `db/seeds.rb` requires `SEED_ADMIN_PASSWORD` with no fallback — a committed default made any reachable dev instance a trivial admin takeover.
- Draft disclosure: every autocomplete query (`autocomplete_query` helper) targets **published** recipes only; blank/non-String `q` yields `[]`, never a full enumeration. Responses are capped at `Recipe::AUTOCOMPLETE_LIMIT` (20), and search terms run through `ActiveRecord::Base.sanitize_sql_like` (via the `like_pattern` helper on `ApplicationController`, and directly in `Filters::{Name,Author,Ingredients}Filter` and `Recipe.fuzzy_autocomplete_for`). The values were always bound parameters, so this is not an injection fix — it stops a literal `%` or `_` acting as a wildcard and returning the whole table.
- Mobile/browser compatibility: layouts use `min-h-dvh` rather than `min-h-screen`, since `100vh` overshoots on mobile browsers with dynamic chrome; the marble background is painted by a fixed `body::before` pseudo-element because iOS Safari ignores `background-attachment: fixed` with `background-size: cover`; `.lightbox-overlay` carries `-webkit-backdrop-filter` for Safari < 18; numeric recipe fields set `inputmode` (`quantity` uses `decimal`, not `type=number`, because it accepts `"1 1/2"`, `"2-3"`, `"to taste"`); the ingredient row grid stacks below `md`, and each ingredient field carries a real `<label>` hidden at `md`+ with `md:sr-only` — the desktop column headers are plain `<div>`s that were never associated with their inputs, so those fields previously had no accessible name at any width.
...
| `SEED_ADMIN_PASSWORD` | Development | Password for the seeded admin user. **Required** — `db/seeds.rb` raises without it rather than falling back to a committed default. Must satisfy `Devise.password_length` (12+) |
